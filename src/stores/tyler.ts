...
export const useTylerStore = defineStore("tyler", () => {
  // Tyler1 Live Event Data
  const accounts = ["Tyler1#11151"];
  ...
  const getData = async (tag: string) => {
    ...
    try {
      let all = await getAllSeasonGames(tag, 21);
  ...
  const matches = computed(() => {
    return accounts
      .reduce(
        (s: any[], a: any) => [
          ...s,
          ...(data.value[a]?.season[Race.Orc].matches.filter((m: any) =>
  ...
  const race = computed(() => {
    ...
    const pros = {
      wins: _groupBy(
        wins.filter((w: any) => w.teams[1].players[0].oldMmr >= 1800),
        (w) => w.teams[1].players[0].race,
      ),
      loss: _groupBy(
        loss.filter((l: any) => l.teams[0].players[0].currentMmr >= 1800),
  ...
  const prediction = computed(() => {
    const r = accounts.reduce((result, account) => {
      ...
      if (loaded.value >= accounts.length) {
        const d = Math.ceil(
          numberOfGames(
            2000 - data.value[account].season.summary.mmr.current,
            averageGain,
          ) /
            (c / daysSinceStart),
        );
