              ...
              <v-row>
                <v-col cols="6" class="text-center">
                  <div class="text-h5">
                    ...
                    {{ Math.ceil(stats.player.season.total) }}
                  </div>
                </v-col>
                <v-col cols="6" class="text-center">
                  <div class="text-h5">
                    {{ Math.ceil(stats.player.season.total / days) }}
                    game(s) per day on average
                  </div>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12" v-if="stats.player.season.total">
                  <span class="title">Season Total</span>
                  <ResultChart :result="stats.player.season" />
                </v-col>
                    ...
                    <v-row>
                      <v-col cols="12" class="pa-0 pt-2">
                        <span class="ml-2 text-h6">
                          <span
                            :class="{
                              'text-green': stats.player?.season?.mmr.diff > 0,
                              'text-red': stats.player?.season?.mmr.diff < 0,
                            }"
                          >
                            <span v-if="stats.player?.season?.mmr.diff > 0"
                              >+</span
                            >
                            {{ stats.player?.season?.mmr.diff }}
                          </span>
                          This Season
                        </span>
                      </v-col>
              ...
              <v-col cols="12">
                <v-list lines="one" style="overflow: hidden">
                  <v-list-item :prepend-avatar="raceIcon[Race.Human]">
                    <ResultChart
                      :result="stats.player.season.race[Race.Human]"
                    />
                  </v-list-item>
                  <v-list-item :prepend-avatar="raceIcon[Race.Orc]">
                    <ResultChart :result="stats.player.season.race[Race.Orc]" />
                  </v-list-item>
                  <v-list-item :prepend-avatar="raceIcon[Race.NightElf]">
                    <ResultChart
                      :result="stats.player.season.race[Race.NightElf]"
                    />
                  </v-list-item>
                  <v-list-item :prepend-avatar="raceIcon[Race.Undead]">
                    <ResultChart
                      :result="stats.player.season.race[Race.Undead]"
                    />
                  </v-list-item>
                  <v-list-item :prepend-avatar="raceIcon[Race.Random]">
                    <ResultChart
                      :result="stats.player.season.race[Race.Random]"
                    />
                  </v-list-item>
                </v-list>
              </v-col>
        ...
        <v-col cols="5">
          <v-sheet class="pa-4" :elevation="5">
            <v-row>
              ...
              <v-col cols="12">
                <div v-if="stats.player.season.total">
